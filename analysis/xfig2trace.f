...
! Use the last **rectangle or closed quadrilateral** in file as the
! corners of the plotted region, corresponding to xmin, xmax, ymin, ymax
! = xyia(8).

! If a comment exists in the xfig file starting "Box:" 
! (entered by doing an xfig edit of an object and typing it in)
! then read 8 values x,y,x,y,x,y,x,y from this comment into xyia. 
...
! Old transform useful only for rectangle.
...
!---------------------------------------------------------------
! Transform for general quadrilateral.
      
         do i=1,il
            do j=1,nl(i)
               x=xtraces(j,i)
               y=ytraces(j,i)
! Perpendicular distance from line 1-2
     ...
     $              +(xrect(3)-xrect(2))**2)
! Perpendicular distance from line 3-4
...
!               write(*,*)p1,p2,p3,p4
!               write(*,*)f1,g1,f2,g2
!               write(*,*)i,j,xtraces(j,i),ytraces(j,i)
            enddo
         enddo
! Kludge up the xyia
         xyia(1)=xq(1)
