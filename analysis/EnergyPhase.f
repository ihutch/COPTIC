...
c At the same time get the potential u, and calculate energy \int E^2dx
      ...
      integer nfiles,j,k,nsmooth,knmax,idk
      parameter (nfiles=10000)
      real energy(nfiles),time(nfiles),xlen,dlnen(nfiles),smooth(nfiles)
...
c Set the starting number of filewriting to be N
 ...
 6          write(*,*)'Garbled -ns flag (needs number):',phasefilename
         ...
         elseif(phasefilename(1:2).eq.'-q')then
            ...
            j=j+1
            if(j.gt.nfiles)then
               write(*,*)'File number exceeds nfiles',nfiles
               goto 5
            endif
