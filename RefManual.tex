...
\paragraph{-ds....}
Set the timestep subcycle-fraction. Default 0.  If non-zero,
sets the maximum permissible momentum increment per step arising from
electric field. If exceeded for any particle, divide the timestep by
an integer, and take that many shorter steps of duration: $dtc = dt/max(1,
int(f*dt/subcycle))$, where $f$ is the local acceleration of this
particle.
...
the particle and potential files so as to initialize, and start
the step count at 1; bit-2 set [2 or usually 3]: read the flux file and
particle and potential and start at the next step after the prior
run. Normal full restart is -fs3. If bit-3 set [4]: then use the
basename ``restartfile'' for the files from which to restart.
...
executable. If it not, then it will build a fully functional serial
