...
\paragraph{2($\times256=512$) Point-Charge} This spherical object is a point charge,
...
\paragraph{4($\times256=1024$) Insulating Object} The boundary condition is
...
Periodicity of the region for particles can also be set with the
special object \verb!98!, in the form of a line such as
\begin{verbatim}
98,1,0,0
\end{verbatim} which is equivalent to the switch \verb!-pp1,0,0!
Each non-zero value sets particle periodicity in that dimension.
...
\paragraph{-pp....} Set the values of the particle-periodicity for
three dimensions. For each non-zero-valued dimension, particles that
leave the mesh are relocated at the corresponding periodic position
within the mesh, rather than reinjected.
