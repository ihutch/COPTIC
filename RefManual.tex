...
boundary conditions on faces. If \emph{any} are used, here or on the
command line, then \emph{all} faces use the rectangular conditions.
Robin boundary conditions, referring to the outward normal ${\bf n}$
...
superceded and disabled by the -BF type below if present.) The single
...
potential for that dimension. Overrides others if on.
...
the Cyclotronic mover is used.  The
