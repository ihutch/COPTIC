...
regarded as an argument line, and is stored, over-writing any earlier
such argument lines (so only the last such line is used). The text
following the colon is parsed for additional command line switches
before starting on the actual command line arguments. This makes it
possible, if desired, to put all of the settings that control a run
into a single object file, and require no switches explicitly on the
command line.
...
The last line of the object file that starts \verb!Arguments:! (if
any) may be used to add additional command line switches that are
parsed \emph{before} those that actually appear on the command line.
These switches, like those that actually appear on the command line,
override other settings in the object file (even settings that appear
after the line \verb!Arguments: ...!).
...
\paragraph{-En....} Set the fraction of the relative drift between
