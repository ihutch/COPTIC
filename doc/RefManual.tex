...
(\verb!boltzamp!). This should be applied only when both electrons and
ions are being modelled as particles (more than one particle species
is present) and should occur \emph{after} the -sp switch. It is
physically equivalent to adding an additional species whose density is
equal to $f_be\phi/T_e$. That is, like linearized Boltzmann electrons
neutralized by an equal background density of immobile positive
charge. It is implemented through the function \verb!faddlin!. The
