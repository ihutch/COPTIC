! Mostly for testing.
! Assumed 3-D routine, plots representation of the cij/obj data.
...
!      character*40 mystring
      ...
      iyud=iuds(2)
      izud=iuds(3)
! If iosw > 0, then wireframe the objects corresponding to its bits.
! If iosw < 0, plot the sticks as well as the wireframes.
! If iosw = 0, plot only sticks for everything.
      if(iosw.gt.0)then
         istick=0
         iwire=1
         mysw=iosw*2
      ...
      do i=1,ixud
         do j=1,iyud
            do k=1,izud
               ...
               if(iobj.ne.0)then
! Draw joins from the point to the fraction distance.
                  xx(1)=xn(ixnp(1)+i)
                  xx(2)=xn(ixnp(2)+j)
                  xx(3)=xn(ixnp(3)+k)
...
!                  write(*,*)'Calling color',iobj,iobjcode,isob,mysw
                  call color(iobjcode)
               endif
               if(isob.ne.0)then
! Origin point
                  ...
                  ijk(3)=k
! This assumes 3-D.
! Draw joins between fractions common to a single node
! in plane normal to id.
                  do id=1,ndims
                     i1=mod(id,3)+1
                     i2=mod(id+1,3)+1
                     ipen=0
     ...
     $                       +ipx(ipm)+ipy(ipm))+1
!                        write(*,*)'iinter',idob_cij(iinter_cij,iobj)
                        frac=dob_cij(no,iobj)
                        if(frac.lt.1. .and. frac.ge.0.)then
! This a true intersection. Draw to it.
                           xt(id)=xx(id)
                           xt(i1)=xx(i1)+frac*
     $                          (xn(ixnp(i1)+ijk(i1)+irx(ipm))-xx(i1))
                           xt(i2)=xx(i2)+frac*
     $                          (xn(ixnp(i2)+ijk(i2)+iry(ipm))-xx(i2))
!                           write(*,*)no,id,i1,i2,frac,ipm,xt
                     ...
                     enddo
                  enddo
! Draw joins between intersections of adjacent parallel lattice legs
                  do id=1,ndims
!                     id=mod(idd-1,3)+1
...
! Double intersections. Connect them.
!                        call color(iobjcode)
                           xt(id)=xx(id)
                           xt(i1)=xx(i1)+frac*
     $                          (xn(ixnp(i1)+ijk(i1)+irx(ipm))-xx(i1))
...
!                           write(*,*)no,id,i1,i2,frac,ipm,xt
         ...
         enddo
      enddo
! Plot orbits if there are any. (Dummy in solver.)
      call orbit3plot()
! User interface:
      iprinting=0
 56   call eye3d(isw)
