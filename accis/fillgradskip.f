...
c         write(*,*)'Color range does not intersect triangle.'
      ...
      ll=hmin-1
c      lmin=max(int(hmin),min(ng0,ng1))
      lmin=max(int(hmin/iskip),min(ng0,ng1)/iskip)*iskip
      lmax=min(int(hmax+iskip),max(ng0,ng1))
...
c Test for significant distance.
            dd1=(wx2nx(xp(1))-wx2nx(xp(4)))**2 +
     $           (wy2ny(yp(1))-wy2ny(yp(4)))**2 
            dd2=(wx2nx(xp(2))-wx2nx(xp(3)))**2 +
     $           (wy2ny(yp(2))-wy2ny(yp(3)))**2 
            dsig=6.e-8
            if(l.ge.lmax .or. (dd1.gt.dsig .or. dd2.gt.dsig))then
...
c Initialize mid point.
                  ...
                  np=5
c Have to do >=5-vector always, else some pixels are missed, when 
c we use implied closure of a quadrilateral. Don't know why.
               endif
               xp(np)=xp(1)
               yp(np)=yp(1)
               zp(np)=zp(1)
...
c isw2 set => d is direction vector, else absolute value.
      subroutine gradquad4(xq,yq,zq,dq,zg0,zg1,ng0,ng1,isw)
