...
GRADCOLOR(icol) Set current color from a 0-239 color gradient range.
...
\item{P}       Save current position
\item{Q}       Return to saved position (and save current)
\item{O}       Toggle overbar mode (write high, full size)
\item{N}       Toggle underbar mode
}
...
\beginsection{Projected 3-D surface routines}
...
SURF3D(xa,ya,za,iL,ix,iy,iw,worka) Draw an opaque 3-d surface of za(ix,iy),
(leading dimension iL) at xa,ya.  View obtained from file eye.dat (3
reals) or default if nonexistent. Switch iw: .lt.0 no axes; lowest
nibble =0  use last scale, =1 scale to fit x,y (1-D), =2 scale to fit
x,y (2-D). Second byte (*256) color of web if non-zero. Third byte
(*65536) color of axes (if !=0). (Remains set). Second nibble (16+) is
isw for the SURFDR3 call. Array worka(0:iL+1,0:iy+1) must be provided.
Surface is normally (isw=0) drawn as quadrilaterals filled with color
from gradient according to (za) height scaled to the total height.
...
\beginsection{Three-Dimensional Perspective Drawing}
...
xt,zt.  If ifl=0 transform (x,y,z) to (xt,yt). If ifl=-1 return the 
current point in (x,y,z) and eye position in (xt,yt,zt).
...
\item{}call eye3d(isw)
\item{}if(isw.ne.ichar('q') .and. isw.ne.0)goto 51\par
...
ROTATEZOOM(isw) Respond to return of interactive keypress from eye3d, by
rotating the viewpoint by small angles right (r) or left (e), or by
moving eye in (i) or out (o), or by magnifying (z) or shrinking (x) the
unit cube (effectively changing the field of view).


\beginsection{Plotting volumetric 3-D Data}

SLICEGWEB(ifullv,iudsv,u,nw,zp,ixnpv,xnv,ifix,cutitle) Plot as 3-D webs
slices through 3-D array u. ifullv(3) and iudsv(3) are the full and used
dimensions of u. zp(nw,nw) is a work array; nw should be
$>$max(iudsv). ixnpv(3) are the pointers within xnv to the node position
array starts. That is, the positions of the nodes in dimension id are
stored in xnv(ixnpv(id))+1 to xnv(ixnpv(id))+ iudsv(id). ifix is the
initial fixed dimension (1,2 or 3). cutitle is the character array
title of u. Keyboard control of the slice is provided, together with
perspective view controlled by mouse.

SLICEGCONT(ifullv,iudsv,u,nw,zp,ixnpv,xnv,ifixptv,cutitle) Plot perspective
view of slices on which are contours of 3-D array
u(ifullv(1),ifullv(2),ifullv(3)). ifullv(3), iudsv(3), zp(nw,nw), ixnpv(3),
cutitle are as above. The initial intersection of the 3 fixed planes
is at ifixptv(3). Slightly different keyboard/mouse control is
provided.
...
If ipen=-1, then just draw a single point (pixel) at (x,y). 

VECN(x,y,ipen) Draw norm-unit vector to (x,y) with pen up (0), down
(1), point (-1).
...
range (min,max). Then (first+i*delta) are the tick values, but one
might label them with (first+i*delta)/fac10 multiplied by 10$^{ipow}$.
...
For X window systems, vecx.c requires the Xlib and Xt libraries, calling
for compiler/linker arguments such as  -L/usr/X11R6/lib/ -lXt
-lX11. The driver vecglx.c requires GL and GLU, instead of Xt.
